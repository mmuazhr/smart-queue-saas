  ...
  try {
    ...
    const [todayOrders, yesterdayOrders] = await Promise.all([
      prisma.order.findMany({
        where: { storeId: store.id, status: { not: "CANCELLED" }, createdAt: { gte: todayStart, lte: todayEnd } },
        select: { total: true, createdAt: true },
      }),
      prisma.order.findMany({
        where: { storeId: store.id, status: { not: "CANCELLED" }, createdAt: { gte: yesterdayStart, lte: yesterdayEnd } },
        select: { total: true },
      }),
    ]);
    ...
    const topItems = await prisma.orderItem.groupBy({
      ...
      where: { order: { storeId: store.id, status: { not: "CANCELLED" } } },
    ...
    const hourlyData = Array.from({ length: 24 }, (_, i) => ({
      hour: i, label: `${i}:00`, count: 0, revenue: 0,
    }));
